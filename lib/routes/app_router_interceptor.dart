...
class AppRouterInterceptor {
  ...
  FutureOr<String?> redirect(BuildContext context, GoRouterState state) async {
    final bool isSignedIn =
        _ref.read(supabaseServiceProvider).auth.currentSession != null;

    if (!isSignedIn) {
      // sign in 으로 가야만 하는 상태입니다.
      if (state.fullPath?.startsWith(Routes.signIn.name) == false) {
      ...
      if (state.fullPath != null &&
          state.fullPath!.startsWith(Routes.signIn.name)) {
